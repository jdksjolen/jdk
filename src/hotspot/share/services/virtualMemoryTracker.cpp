...
address NewVirtualMemoryTracker::thread_stack_uncommitted_bottom(TrackedRange& rng,
...
void NewVirtualMemoryTracker::snapshot_thread_stacks() {
  ...
  thread_stacks->clear();
  for (int i = 0; i < merged_thread_stacks.length(); i++) {
    thread_stacks->push(merged_thread_stacks.at(i));
  }
}
  ...
  */
  ...
  for (int res_rng_idx = 0; res_rng_idx < res_rngs.length(); res_rng_idx++) {
    ...
    if (rng.flag == MEMFLAGS::mtThreadStack) {
      for (int i = 0; i < thread_stacks->length(); i++) {
        Range& tsr = thread_stacks->at(i);
        if (tsr.start >= rng.start && tsr.end() <= rng.end()) {
          output->print("\n\t");
          print_virtual_memory_region("committed", tsr.start, tsr.size);
        }
      }
...
void NewVirtualMemoryTracker::set_view_region_type(const PhysicalMemorySpace& space,
                                                   address base_addr, MEMFLAGS flag) {
...
void NewVirtualMemoryTracker::remove_all_views_into_space(const PhysicalMemorySpace& space) {
  reserved_regions->at(space.id).clear_and_deallocate();
}

void NewVirtualMemoryTracker::remove_view_into_space(const PhysicalMemorySpace& space,
...
void NewVirtualMemoryTracker::init() {
...
void NewVirtualMemoryTracker::merge_committed(RegionStorage& ranges) {
...
void NewVirtualMemoryTracker::sort_regions(OffsetRegionStorage& storage) {
...
NewVirtualMemoryTracker::OverlappingResult
NewVirtualMemoryTracker::overlap_of(TrackedOffsetRange to_split, Range to_remove,
