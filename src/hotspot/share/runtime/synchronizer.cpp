...
void ObjectSynchronizer::wait_uninterruptibly(Handle obj, JavaThread* current) {
  assert(!ObjectMonitorMode::java_only(), "sanity");
  // We can use this with NewMonitorPolicy==0 because no stack-locking is
  // involved and we always inflate. But we should do this via a VM-Java-VM call.
...
void ObjectLocker::wait_uninterruptibly() {  // wait forever
  assert(_do_lock, "incorrect call to wait() with no locking");
  if (!ObjectMonitorMode::java_only()) {
    // We can call this with UseNewMonitors as it always inflates,
    // and there is no existing VM interface for wait_uninterruptibly
    // so we don't call up to Java first ... though we probably should. FIXME!
    ObjectSynchronizer::wait_uninterruptibly(_obj, _thread);
  } else {
    assert(!_thread->has_pending_exception(), "unexpected pending exception in wait_uninterruptibly");
    // Weak only restores old exception if there is no exception.
    // Any exception from wait_ui will thus be kept.
    WeakPreserveExceptionMark pem(_thread);
    ObjectSynchronizer::java_wait_uninterruptibly(_obj, _thread);
  }
}
