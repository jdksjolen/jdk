...
bool AsyncLogWriter::write(AsyncLogMap<AnyObj::RESOURCE_AREA>& snapshot,
  ...
  int req = 0;
  CircularLogBuffer::Message msg;
  while (_circular_buffer.has_message()) {
    CircularLogBuffer::DequeueResult result = _circular_buffer.dequeue(&msg, write_buffer, write_buffer_size);
    assert(result != CircularLogBuffer::DequeueResult::NoMessage, "Race detected but there is only one reading thread");
    if (result == CircularLogBuffer::DequeueResult::TooSmall) {
