...
CircularMapping::CircularMapping(size_t size)
  : _buffer(nullptr),
    _size(size) {
  _buffer = os::reserve_memory(size, false, mtLogging);
  if (_buffer == nullptr) {
    vm_exit_out_of_memory(size, OOM_MMAP_ERROR, "%s", allocation_failure_msg);
  }
  if (!os::commit_memory(_buffer, size, false)) {
    vm_exit_out_of_memory(size, OOM_MMAP_ERROR, "%s", allocation_failure_msg);
  }
}
...
void CircularStringBuffer::enqueue_locked(const char* str, size_t size, LogFileStreamOutput* output,
  ...
  assert(unused >= sizeof(Message), "Invariant: always have space for a flush token");
#endif
...
void CircularStringBuffer::await_message() {
  while (true) {
    ConsumerLocker cl(this);
    while (_head == _tail && _stalled_message == nullptr) {
      _consumer_lock.wait(10);
    }
    break;
  }
}
