...
jint ShenandoahHeap::initialize() {
  ...
  if (!_heap_region_special) {
    os::commit_memory_or_exit(sh_rs.base(), _initial_size, heap_alignment, sh_rs.flag(), false,
  ...
  MemTracker::record_virtual_memory_type(bitmap.base(), bitmap.size(), mtGC);
  ...
  if (!_bitmap_region_special) {
    os::commit_memory_or_exit((char *) _bitmap_region.start(), bitmap_init_commit, bitmap_page_size, mtGC, false,
  ...
  if (ShenandoahVerify) {
    ...
    if (!verify_bitmap.special()) {
      os::commit_memory_or_exit(verify_bitmap.base(), verify_bitmap.size(), bitmap_page_size, mtGC, false,
                                "Cannot commit verification bitmap memory");
    }
    MemTracker::record_virtual_memory_type(verify_bitmap.base(), verify_bitmap.size(), mtGC);
  ...
  MemTracker::record_virtual_memory_type(aux_bitmap.base(), aux_bitmap.size(), mtGC);
  ...
  MemTracker::record_virtual_memory_type(region_storage.base(), region_storage.size(), mtGC);
  if (!region_storage.special()) {
    os::commit_memory_or_exit(region_storage.base(), region_storage_size, region_page_size, mtGC, false,
...
bool ShenandoahHeap::prepare_aux_bitmap_for_iteration() {
  ...
  if (!_aux_bitmap_region_special && !os::commit_memory((char*)_aux_bitmap_region.start(), _aux_bitmap_region.byte_size(), mtGC, false)) {
...
bool ShenandoahHeap::commit_bitmap_slice(ShenandoahHeapRegion* r) {
  ...
  if (!os::commit_memory(start, len, mtGC, false)) {
    return false;
  }
