 ...
 *          java.management
 * @run main/othervm -XX:NativeMemoryTracking=light JcmdScaleLightweight
 */
...
public class JcmdScaleLightweight {
  ...
  public static void main(String args[]) throws Exception {
    ...
    pb.command(new String[] { JDKToolFinder.getJDKTool("jcmd"), pid, "VM.native_memory", "light", "scale=GB"});
    output = new OutputAnalyzer(pb.start());
    output.shouldContain("GB, committed=");

    pb.command(new String[] { JDKToolFinder.getJDKTool("jcmd"), pid, "VM.native_memory", "light", "scale=apa"});
    output = new OutputAnalyzer(pb.start());
    output.shouldContain("Incorrect scale value: apa");

    pb.command(new String[] { JDKToolFinder.getJDKTool("jcmd"), pid, "VM.native_memory", "light", "scale="});
    output = new OutputAnalyzer(pb.start());
