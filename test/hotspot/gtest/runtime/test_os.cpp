...
static inline bool can_reserve_executable_memory(void) {
  ...
  if (exec_supported) {
    os::release_memory(p, len, mtTest);
  }
...
static void carefully_release_multiple(address start, int num_stripes, size_t stripe_len) {
  for (int stripe = 0; stripe < num_stripes; stripe++) {
    ...
    EXPECT_TRUE(os::release_memory((char*)q, stripe_len, mtTest));
  }
}
...
static address reserve_multiple(int num_stripes, size_t stripe_len) {
  ...
  for (int tries = 0; tries < 256 && p == NULL; tries ++) {
    ...
    // .. release it...
    EXPECT_TRUE(os::release_memory((char*)p, total_range_len, mtTest));
    ...
    for (int stripe = 0; stripe < num_stripes; stripe++) {
      ...
      if (q == NULL) {
        ...
        EXPECT_TRUE(os::commit_memory((char*)q, stripe_len, mtTest, executable));
      }
    }
  }
  return p;
...
static address reserve_one_commit_multiple(int num_stripes, size_t stripe_len) {
  ...
  for (int stripe = 0; stripe < num_stripes; stripe++) {
    address q = p + (stripe * stripe_len);
    if (stripe % 2 == 0) {
      EXPECT_TRUE(os::commit_memory((char*)q, stripe_len, mtTest, false));
    }
  }
  return p;
}
...
TEST_VM(os, release_multi_mappings) {
  ...
  const size_t middle_stripe_len = (num_stripes - 2) * stripe_len;
  {
    ...
    ASSERT_TRUE(os::release_memory((char*)p_middle_stripes, middle_stripe_len, mtTest));
  }
  PRINT_MAPPINGS("B");
  ...
  // Clean up. Release all mappings.
  {
    ...
    ASSERT_TRUE(os::release_memory((char*)p, total_range_len, mtTest));
  }
}
#endif // !AIX
...
TEST_VM(os, release_bad_ranges) {
  ...
  ASSERT_FALSE(os::release_memory(p, M, mtTest));
  // Release part of range
  ASSERT_FALSE(os::release_memory(p + M, M, mtTest));
  ...
  //  accidentally release neighbors)
  {
    ...
    ASSERT_FALSE(os::release_memory(p, M * 5, mtTest));
    ASSERT_FALSE(os::release_memory(p - M, M * 5, mtTest));
    ASSERT_FALSE(os::release_memory(p - M, M * 6, mtTest));
  }

  ASSERT_TRUE(os::release_memory(p, 4 * M, mtTest)); // Release for real
  ASSERT_FALSE(os::release_memory(p, 4 * M, mtTest)); // Again, should fail
}
#endif // _WIN32
...
TEST_VM(os, release_one_mapping_multi_commits) {
  ...
  address p2 = (address)os::attempt_reserve_memory_at((char*)p, total_range_len, mtTest);
  ASSERT_EQ(p2, p);
  PRINT_MAPPINGS("C");
...
TEST_VM(os, show_mappings_full_range) {
  ...
  if (p != nullptr) {
    if (os::commit_memory(p, 1 * M, mtInternal, false)) {
      strcpy(p, "ABCDEFGHIJKLMNOPQRSTUVWXYZ");
    }
  }
  test_show_mappings(nullptr, 0);
  if (p != nullptr) {
    os::release_memory(p, 1 * M, mtTest);
  }
}
