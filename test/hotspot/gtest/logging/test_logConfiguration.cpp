...
TEST_VM_F(LogConfigurationTest, describe) {
  ...
  EXPECT_PRED2(string_contains_substring, description, stdout_log()->name());
  EXPECT_PRED2(string_contains_substring, description, stderr_log()->name());
...
TEST_VM_F(LogConfigurationTest, disable_logging) {
  ...
  for (LogTagSet* ts = LogTagSet::first(); ts != NULL; ts = ts->next()) {
    EXPECT_FALSE(ts->has_output(stdout_log()));
...
TEST_VM_F(LogConfigurationTest, parse_empty_command_line_arguments) {
  ...
  for (size_t i = 0; i < ARRAY_SIZE(empty_variations); i++) {
    ...
    for (LogTagSet* ts = LogTagSet::first(); ts != NULL; ts = ts->next()) {
      EXPECT_EQ(LogLevel::Unspecified, ts->level_for(stdout_log()));
    }
  }
}
...
TEST_VM_F(LogConfigurationTest, configure_stdout) {
  ...
  EXPECT_EQ(LogLevel::Info, logging_ts->level_for(stdout_log()));
  ...
  for (LogTagSet* ts = LogTagSet::first(); ts != NULL; ts = ts->next()) {
    if (ts->contains(PREFIX_LOG_TAG(gc))) {
      if (ts->ntags() == 1) {
        EXPECT_EQ(LogLevel::Debug, ts->level_for(stdout_log()));
      } else {
        EXPECT_EQ(LogLevel::Off, ts->level_for(stdout_log()));
      }
    }
  }
  ...
  for (LogTagSet* ts = LogTagSet::first(); ts != NULL; ts = ts->next()) {
    if (ts->contains(PREFIX_LOG_TAG(gc))) {
      EXPECT_EQ(LogLevel::Trace, ts->level_for(stdout_log()));
    } else if (ts == logging_ts) {
      // Previous setting for 'logging' should remain
      EXPECT_EQ(LogLevel::Info, ts->level_for(stdout_log()));
    } else {
      EXPECT_EQ(LogLevel::Off, ts->level_for(stdout_log()));
    }
  }
