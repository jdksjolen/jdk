...
#undef LOG_LEVEL
  ...
  void test_asynclog_drop_messages() {
    ...
    // Grab read lock before flushing to guarantee that we overflow the buffer.
    AsyncLogWriter::instance()->_circular_buffer._read_lock.lock();
    lm.flush();
    AsyncLogWriter::instance()->_circular_buffer._read_lock.unlock();
  }
  ...
  void test_room_for_flush() {
    ...
    CircularLogBuffer::StatisticsMap map;
    CircularLogBuffer cb(map, lock, os::vm_page_size());
    const size_t count = (cb._buffer.size / (strlen(large_message)+1 + sizeof(CircularLogBuffer::Message))) - 1;
    stringStream ss;
    ss.print("file=%s", TestLogFileName);
    LogFileOutput out(ss.freeze());
    for (size_t i = 0; i < count; i++) {
      cb.enqueue_locked(large_message, strlen(large_message), &out, CircularLogBuffer::None);
    }
    ...
    cb.enqueue_locked("", 0, nullptr, CircularLogBuffer::None);
